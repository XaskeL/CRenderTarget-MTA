CRenderTarget =
{
	...
	-- Получает приблизительный размер RT с нужными размерами и параметрами ( bAlpha ) в памяти.
	-- = number
    GetWeightFromSize       = function ( this, iWidth, iHeight, bAlpha )
        local iPixels = ( iWidth * iHeight );
        return ( bAlpha and ( iPixels * this.m_pBytesPerPixel [ 1 ] ) or ( iPixels * this.m_pBytesPerPixel [ 2 ] ) ) / 1e+6;
    end;
    ...
    GetPixelsFromWeight     = function ( this, iMiB, bAlpha )
        ...
        return tonumber( ( bAlpha and ( iBytes / this.m_pBytesPerPixel [ 1 ] ) or ( iBytes / this.m_pBytesPerPixel [ 2 ] ) ) );
    end;
	...
	-- Позволяет узнать, можно ли поместить указанный RT ( iWidth, iHeight, bAlpha ) в память.
	-- = bool
    IsCanPutInToRAM         = function ( this, iWidth, iHeight, bAlpha )
        return dxGetStatus()["VideoMemoryFreeForMTA"] > this:GetWeightFromSize( iWidth, iHeight, bAlpha );
    end;
};
...
print( "IsCanPutInToRAM:", tostring( CRenderTarget:IsCanPutInToRAM( 1920, 1080, true ) ) );
--]]
